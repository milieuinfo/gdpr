...
(function() {
	...
	function GDPR() {
		...
		if (getScriptBooleanAttribute('auto-open', false)
				&& Object.keys(optIns).length > 0) {
			open();
		}

		function addOptIn(name, label, description, value, required,
				activationCallback, deactivationCallback) {
			if (!optIns[name]) {
				optIns[name] = {
					'name' : name,
					'label' : label,
		...
		function addFunctionalOptIn() {
			addOptIn(
					'functional',
					'Noodzakelijke cookies toestaan (verplicht)',
		...
		function addAnalyticsOptIn() {
			addOptIn(
					'analytics',
					'Anonieme gebruikersstatistieken toestaan',
					'Dit soort cookies helpt ons te begrijpen hoe bezoekers de website gebruiken, door anoniem gegevens te verzamelen en te rapporteren.',
					true, false, function() {
		...
		function initialize() {
			...
			getGDPRScript().getAttributeNames().forEach(
				function(attributeName) {
					var matches = /^data-opt-in-([^-]+)(-(.+))?$/.exec(attributeName);
					if (matches) {
						var name = matches[1];
						addOptIn(name, getOptInLabelAttribute(name),
								getOptInDescriptionAttribute(name),
								getOptInValueAttribute(name),
								getOptInRequiredAttribute(name));
					}
				}
			);
		}
		...
		function getOptInAttribute(name, attribute, fallback) {
			return getScriptDataAttribute("opt-in-" + name + "-" + attribute,
					fallback);
		}

		function getOptInBooleanAttribute(name, attribute, fallback) {
			return getScriptBooleanAttribute(
					"opt-in-" + name + "-" + attribute, fallback);
		}
		...
		function getMatomoId() {
			...
			if (!match) {
				console
						.error('de website is nog niet gekend bij ons dus er zullen geen gebruikersstatistieken bijgehouden worden');
			}
			return match;
		}
		...
		function getStyleLink() {
			...
			link.setAttribute('href', 'data:text/css;charset=UTF-8,'
					+ encodeURIComponent(getStyle()));
			return link;
		}
		...
		function setCookie(name, value) {
			document.cookie = cookiePrefix + name + '=' + value
					+ ';2147483647;path=/;';
		}
